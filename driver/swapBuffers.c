/*
	Code referenced from https://github.com/Microsoft/Windows-driver-samples/tree/master/filesys/miniFilter/swapBuffers
	Modification done
		[x] to allow userland process to communicates with driver
		[x] to determine which files need to be "tampered"
*/
#include <fltKernel.h>
#include <dontuse.h>
#include <suppress.h>
...
#define NTDEVNAME L"\\Device\\Cozen"
#define DOSDEVNAME L"\\DosDevices\\Cozen"
#define IOCTL_COZEN_WRITEMEM CTL_CODE(DEVICE_COZEN, 0x901, METHOD_BUFFERED, FILE_READ_ACCESS)

typedef struct _COZEN_READMEM {
	UCHAR drive[26];
} COZEN_READMEM, *PCOZEN_READMEM;
...
typedef struct _COZEN_DEVEXT {
	LIST_ENTRY entry;
} COZEN_DEVEXT, *PCOZEN_DEVEXT;
...
)
{
	...
	ULONG iocode;
	PIO_STACK_LOCATION iosb;
	NTSTATUS status = STATUS_SUCCESS;

	iosb = IoGetCurrentIrpStackLocation(irp);
	iocode = iosb->Parameters.DeviceIoControl.IoControlCode;

	switch (iocode)
	{
		case IOCTL_COZEN_WRITEMEM:
		{
			PCOZEN_READMEM p;
			p = (PCOZEN_READMEM)irp->AssociatedIrp.SystemBuffer;
    ...
    )
{
	...
	RtlInitUnicodeString(&ntdevname, NTDEVNAME);
	...
	DriverObject->MajorFunction[IRP_MJ_DEVICE_CONTROL] = COZENIoctl;
	DriverObject->MajorFunction[IRP_MJ_CREATE] = COZENCreateClose;
	DriverObject->MajorFunction[IRP_MJ_CLOSE] = COZENCreateClose;
	...
	DbgPrint("IoCreateDevice() Created");

	status = IoCreateSymbolicLink(&dosdevname, &ntdevname);
	if (!NT_SUCCESS(status)) {
		DbgPrint("IoCreateSymbolicLink failed");
		return status;
	}
	DbgPrint("IoCreateSymbolicLink() Created");
...
// check if destination is a non whitelisted drive
BOOLEAN isNotWhiteListed(PFLT_CALLBACK_DATA Data, PCFLT_RELATED_OBJECTS FltObjects)
{
	...
	if (NT_SUCCESS(status))
	{
		int driveIndex = volCtx->Name.Buffer[0] - 0x41;
		// if drive is not whitelisted & we ensure we are not tampering with C:
		if (drives[driveIndex] == 0 && driveIndex != 2 && driveIndex >= 0 && driveIndex < 26)
		{
			isBlacklisted = TRUE;
		}
	}
...
--*/
{
    ...
    try {
        ...
        try {
			...
			if (isNotWhiteListed(Data, FltObjects))
			{
